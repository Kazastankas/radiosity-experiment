 ...
 */
static void run_main_loop( void (*update_fn)( LoopData*, bool ), LoopData* data, double ups )
{
    ...
    while ( *data->running ) {
        ...
        if ( frame_rate_counter == FRAME_RATE_PRINT_TIME ) {
            ...
            printf( "frame rate: %f, avg frame time: %f\n",
                FRAME_RATE_PRINT_TIME * 1000 / double(curr_time - frame_rate_counter_start),
                double(total_frame_time) / FRAME_RATE_PRINT_TIME
            );
...
int Application::start_application( Application* app, int width, int height, double fps, const char* title )
{
    LoopData ldata;
