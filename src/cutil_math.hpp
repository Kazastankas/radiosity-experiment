...
#ifndef AED_CUTIL_MATH_H
#define AED_CUTIL_MATH_H
...
// clamp
inline __device__ __host__ int clamp(int x, int a, int b)
{
...
inline __device__ __host__ bool is_nearly_zero( float x )
{
    return fabs( x ) < 1.0e-5f;
}

inline __device__ __host__ float saturatef(float x)
{
#ifdef  __CUDA_ARCH__
...
inline __device__ __host__ float normalize_int( uint8_t x )
{
    return saturatef( ((float)x) / 255.0f );
}

inline __device__ __host__ uint8_t denormalize_int( float x )
{
    return (uint8_t) clamp( x * 255.0f, 0.0f, 255.0f );
}

// uchar2 functions
////////////////////////////////////////////////////////////////////////////////

inline __host__ __device__ uint8_t index(uchar2 a, size_t idx)
{
    return (&a.x)[idx];
}
...
inline __host__ __device__ int index(int2 a, size_t idx)
{
    return (&a.x)[idx];
}

inline __host__ __device__ int& indexp(int2* a, size_t idx)
{
    return (&a->x)[idx];
}

inline __host__ __device__ int component_product( int2 a )
{
    return a.x * a.y;
}
...
inline __host__ __device__ float2 float_from_int( int2 a )
{
    return make_float2( a.x, a.y );
}
inline __host__ __device__ int2 int_from_float( float2 a )
{
...
inline __device__ __host__ float2 saturatef( float2 v )
{
    return make_float2( saturatef( v.x ), saturatef( v.y ) );
}
...
inline __host__ __device__ float2 safe_normalize(float2 v)
{
	float sqLen = dot(v, v);
	if ( sqLen > 1.0e-5 ) {
		float invLen = rsqrtf(sqLen);
		return v * invLen;
	} else {
...
inline __host__ __device__ int2 floori(const float2 v)
{
    return make_int2((int)floor(v.x), (int)floor(v.y));
}
...
inline __host__ __device__ float index(float2 a, size_t idx)
{
    return (&a.x)[idx];
}

inline __host__ __device__ float& indexp(float2* a, size_t idx)
{
    return (&a->x)[idx];
}
...
inline __host__ __device__ float3 float_from_int( int3 a )
{
    return make_float3( a.x, a.y, a.z );
}
inline __host__ __device__ int3 int_from_float( float3 a )
{
...
inline __host__ __device__ float index(float3 a, size_t idx)
{
    return (&a.x)[idx];
}
...
inline __host__ __device__ int3 floori(const float3 v)
{
    return make_int3((int)floor(v.x), (int)floor(v.y), (int)floor(v.z));
}
...
inline __host__ __device__ float& indexp(float3* a, size_t idx)
{
    return (&a->x)[idx];
}
...
inline __host__ __device__ uint8_t index(uchar3 a, size_t idx)
{
    return (&a.x)[idx];
}

inline __host__ __device__ uint8_t& indexp(uchar3* a, size_t idx)
{
    return (&a->x)[idx];
}
...
inline __host__ __device__ uchar4 make_uchar4(uint8_t a)
{
    return make_uchar4(a,a,a,255);
}
...
inline __device__ __host__ float4 saturatef( float4 a )
{
    return make_float4( saturatef( a.x ), saturatef( a.y ), saturatef( a.z ), saturatef( a.w ) );
}

inline __device__ __host__ float4 normalize_int( uchar4 x )
{
    return saturatef( make_float4( x ) / 255.0f );
}

inline __device__ __host__ uchar4 denormalize_int( float4 x )
{
    return make_uchar4( clamp( x * 255.0f, 0.0f, 255.0f ) );
}
...
inline __host__ __device__ int& indexp(int3* a, size_t idx)
{
    return (&a->x)[idx];
}

inline __host__ __device__ int index(int3 a, size_t idx)
{
    return (&a.x)[idx];
}
...
inline __host__ __device__ int component_product( int3 a )
{
    return a.x * a.y * a.z;
}
...
inline __host__ __device__ uint& indexp(uint3* a, size_t idx)
{
    return (&a->x)[idx];
}

inline __host__ __device__ uint index(uint3 a, size_t idx)
{
    return (&a.x)[idx];
}
#endif
